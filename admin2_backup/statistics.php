...
try {
    ...
    $stats['giftcards_pending'] = $pdo->query("
        SELECT COUNT(*) FROM order_items oi
        JOIN products p ON oi.product_id = p.product_id
        WHERE (p.name LIKE '%Tarjeta de Regalo%' OR oi.personalization_name LIKE '%@%')
        AND (oi.giftcard_sent IS NULL OR oi.giftcard_sent = 0)
    ")->fetchColumn();
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-card-header">
                        ...
                        <div class="stat-card-icon" style="background: rgba(106, 90, 205, 0.15); color: #6a5acd;">
                            <i class="fas fa-gift"></i>
                        </div>
                    </div>
                    <div class="stat-card-value"><?php echo number_format($stats['giftcards_pending']); ?></div>
                    <?php if ($stats['giftcards_pending'] > 0): ?>
