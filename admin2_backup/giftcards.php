...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        JOIN orders o ON oi.order_id = o.order_id
        JOIN products p ON oi.product_id = p.product_id
        WHERE personalization_name IS NOT NULL
        AND (p.name LIKE '%Tarjeta de Regalo%' OR oi.personalization_name LIKE '%@%')
...
<body>
                                ...
                                <?php else: ?>
                                    <?php foreach ($giftcards as $giftcard): ?>
                                        <tr>
                                            ...
                                            <td class="text-truncate" title="<?php echo htmlspecialchars($giftcard['personalization_name']); ?>">
                                                <?php echo htmlspecialchars($giftcard['personalization_name']); ?>
                                            </td>
                                            <td><?php echo htmlspecialchars($giftcard['personalization_number']); ?></td>
                                            <td>
            ...
            <div id="transactionsModal" class="modal">
                <div class="modal-content">
                    <div class="modal-header">
                        <h3>Transacciones de Gift Card</h3>
                        <span class="close" onclick="closeTransactionsModal()">&times;</span>
                    </div>
                    <div class="modal-body" id="transactions-list">
    ...
    <script>
        ...
        function loadTransactions(code) {
            ...
            transactionsList.innerHTML = '<p>Cargando transacciones...</p>';
            modal.style.display = 'block';
            
            // Hacer solicitud AJAX para obtener transacciones
            fetch('get_transactions.php?code=' + code)
                ...
                .then(data => {
                    if (data.success) {
                        if (data.transactions.length > 0) {
                            ...
                            transactionsList.innerHTML = '<p>No hay transacciones para esta gift card</p>';
                        }
                    } else {
                        transactionsList.innerHTML = `<p class="error-message">${data.message}</p>`;
                    }
                })
                .catch(error => {
