...
function sendGiftCardEmail($orderData, $giftcardItem) {
    ...
    <head>
        ...
        <title>Tarjeta de Regalo JerSix</title>
        <style>
            body {
    ...
    try {
        ...
        // Usar la función de Gmail SMTP
        $mailSent = sendGmailEmail($recipientEmail, $subject, $htmlMessage);
        
        if ($mailSent) {
            writeLog("Correo de gift card enviado exitosamente a: " . $recipientEmail);
            
            // Ya no actualizamos automáticamente la base de datos
            // Comentado para permitir que el administrador lo haga manualmente
            // $stmt = $pdo->prepare("UPDATE order_items SET giftcard_sent = 1 WHERE order_id = ? AND personalization_name = ?");
            // $stmt->execute([$orderData['order_id'], $recipientEmail]);
            
            return true;
        } else {
            writeLog("ERROR: No se pudo enviar el correo a: " . $recipientEmail);
            
            // Intentar guardar en cola para reintento posterior
            ...
            $mailFile = $mailFolder . '/giftcard_' . $orderData['order_id'] . '_' . time() . '.json';
            if (file_put_contents($mailFile, json_encode($mailData))) {
                writeLog("Gift card guardada en cola: " . $mailFile);
                return true;
            }
        ...
        writeLog("ERROR al enviar correo: " . $e->getMessage());
        return false;
    }
} 